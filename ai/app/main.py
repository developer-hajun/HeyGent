...
async def lifespan(app: FastAPI):
    ...
    _postgres_pool = build_postgres_connection_pool(settings.postgres_dsn, min_size=4, max_size=16)
    postgres_connection_factory = (
        (lambda: pooled_connection_factory(_postgres_pool, settings.postgres_dsn))
        if _postgres_pool is not None
        else (lambda: connect_postgres(settings.postgres_dsn))
    )
