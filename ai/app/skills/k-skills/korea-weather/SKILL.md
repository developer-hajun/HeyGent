...
사용자가 공공데이터포털 기상청 API key를 직접 다룰 필요는 없다. `KSKILL_PROXY_BASE_URL` 이 있으면 그 값을 사용하고, 없으면 공개 hosted proxy `https://k-skill-proxy.nomadamas.org` 를 사용한다. upstream `KMA_OPEN_API_KEY` 는 proxy 서버에서만 관리한다.
...
- proxy route가 내려가 있거나 weather route가 아직 배포되지 않은 경우
