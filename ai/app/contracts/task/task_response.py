...
class PendingApprovalResponse(BaseModel):
    ...
    status: str = Field(description="승인 요청 상태입니다. 보통 `PENDING`이면 사용자 입력을 기다리는 중입니다.")
...
class StepRunSummaryResponse(BaseModel):
    ...
    status: str = Field(description="단계 상태입니다.")
...
class TaskRunFlowActivityResponse(BaseModel):
    ...
    status: str | None = Field(default=None, description="이 event 발생 시점의 상태입니다.")
...
class ActiveTaskRunListItemResponse(BaseModel):
    ...
    source: str = Field(description="목록에 들어온 이유입니다. `active`는 실행/대기 중, `recent`는 방금 끝난 작업입니다.")
    session_key: str | None = Field(default=None, description="sessionId의 내부 저장명입니다.")
    status: str = Field(description="TaskRun 상태입니다.")
...
class TaskEventResponse(BaseModel):
    ...
    status: str | None = Field(default=None, description="event 발생 시점의 TaskRun 또는 StepRun 상태입니다.")
