        ...
        sections.append(
            "\n".join(
                [
                    ...
                    "등록된 에이전트가 수행할 수 있는 정보 조회, 검토, 구현, 테스트 작업은 CEO가 직접 끝내기보다 해당 에이전트에게 맡기세요.",
                    "후보 에이전트가 맡을 수 있는 작업이면 CEO가 web_search, file, browser 도구를 직접 실행하기 전에 먼저 session_agent_task 를 호출하세요.",
                    "CEO가 직접 처리하는 경우는 수행할 수 있는 세션 에이전트가 없거나, 인사말처럼 작업 분리가 필요 없는 매우 작은 응답으로 제한하세요.",
