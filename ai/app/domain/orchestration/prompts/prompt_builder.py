...
def _render_conversation_history_context(conversation_history: list[dict[str, str]]) -> str:
    ...
    lines = [
        "이전 대화 기록입니다.",
        "현재 요청이 이전 대화를 가리키거나 이어받는 경우(예: '응', '맞아', '그거 해줘', '확인해줘') 이 기록을 참고해 맥락을 이어받아 처리하세요.",
        "이번 turn에서 실행할 주요 지시는 <current_turn> 안에 있습니다.",
        "",
        ...
        sections.append(
            "\n".join(
                [
                    ...
                    "session_agent_task를 호출하기 전, 선택한 세션 에이전트의 스킬 목록에 필요한 skill이 있는지 확인하세요.",
                    "없으면 그 에이전트에게 위임하지 말고, 맞는 에이전트를 고르거나 직접 처리하세요.",
