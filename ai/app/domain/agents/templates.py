...
아래 유형은 추가 확인 없이 바로 세션 에이전트 작업으로 위임한다. "확인해드릴게요", "조회해드릴게요" 같은 예고 메시지만 남기고 끝내지 말고, 반드시 `session_agent_task` 도구를 호출해 위임한 뒤 결과를 받아 사용자에게 전달한다.

- **날씨 / 기온 / 강수**: K-에이전트에 위임. 지역 미언급 시 서울로 처리.
- **미세먼지 / 대기질**: K-에이전트에 위임. 지역 미언급 시 서울로 처리.
- **지하철 / 교통 / 버스**: K-에이전트에 위임.
- **한국 생활 정보 전반** (로또, 학교급식, 부동산, 유가, 도서관 등): K-에이전트에 위임.
- **Gmail / 메일 정리**: Gmail 에이전트에 위임.
- **건강 데이터**: 헬스 에이전트에 위임.
- **코드 구현 / 버그 수정**: 개발 에이전트에 위임.
- **QA / 테스트**: QA 에이전트에 위임.

이전 대화에서 주제가 정해진 상태에서 짧은 followup("서울", "응", "ㅇㅇ", "확인해줘", "알려줘")이 오면 이전 맥락을 이어받아 바로 위임한다. 절대 다시 되묻지 않는다.
...
- "확인해드릴게요", "조회해드릴게요", "바로 알려드릴게요" 같은 예고만 하고 실제 tool 호출 없이 답변을 끝내지 않습니다. 반드시 tool을 호출하고 결과를 받아서 전달합니다.
- 날씨, 미세먼지, 교통 등 생활 정보를 묻는데 지역이 빠졌다고 되묻지 않습니다. 서울로 처리하고 결과에 "서울 기준"임을 표시합니다.
""",
        ),
        (
            "TOOLS.md",
...
BUILTIN_AGENT_TEMPLATES: tuple[BuiltinAgentTemplate, ...] = (
    BuiltinAgentTemplate(
        ...
        skills=("skill-index",),
        toolsets=("skills", "session", "planning", "web"),
        documents=(
            (
                "AGENTS.md",
                "기본 지침",
