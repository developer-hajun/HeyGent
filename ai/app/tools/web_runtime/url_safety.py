...
Limitations (documented, not fixable at pre-flight level):
  ...
  - Redirect-based bypass is mitigated by httpx event hooks that re-validate
    ...
    media cache helpers. Web tools use third-party SDKs (Tavily/Exa/Parallel)
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``heygent_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("heygent_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``heygent_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
