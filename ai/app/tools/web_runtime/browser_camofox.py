...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``TOOL_RUNTIME_HOME 아래의 .env``.
"""
...
from app.tools.web_runtime.browser_camofox_state import get_camofox_identity
from app.tools.web_runtime.registry import tool_error
...
def _managed_persistence_enabled() -> bool:
    """Return whether tool-managed persistence is enabled for Camofox.
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the 도구 실행 프로필 so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if _managed_persistence_enabled():
            ...
            session = {
                "user_id": f"heygent_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        try:
            ...
            from app.tools.web_runtime.browser_tool import (
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_tool_home() / "browser_screenshots"
