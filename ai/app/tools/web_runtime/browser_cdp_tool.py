...
# ``websockets`` is a transitive dependency of browser tooling (via fal_client
...
def _resolve_cdp_endpoint() -> str:
    ...
    Delegates to ``app.tools.web_runtime.browser_tool._get_cdp_override`` so precedence stays
    ...
    """
    try:
        from app.tools.web_runtime.browser_tool import _get_cdp_override  # type: ignore[import-not-found]
...
def _browser_cdp_check() -> bool:
    ...
    """
    try:
        from app.tools.web_runtime.browser_tool import (  # type: ignore[import-not-found]
            _get_cdp_override,
