"""Camofox browser profile state helpers.

Provides profile-scoped identity and state directory paths for Camofox
persistent browser profiles. When managed persistence is enabled, the runtime
...
def get_camofox_state_dir() -> Path:
    ...
    return get_tool_home() / CAMOFOX_STATE_DIR_NAME / CAMOFOX_STATE_SUBDIR


def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Return the stable Camofox identity for this profile.

    The user identity is profile-scoped so the same profile reuses cookies.
    ...
    return {
        "user_id": f"heygent_{user_digest}",
