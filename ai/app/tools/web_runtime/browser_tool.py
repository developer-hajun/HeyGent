#!/usr/bin/env python3
"""
브라우저를 열고 페이지 이동, 클릭, 입력, 스크린샷 분석을 실행하는 모듈이다.

LLM이 `browser_navigate`, `browser_click`, `browser_type` 같은 도구를 호출하면
public adapter가 이 파일의 함수를 부른다. 로컬 Chromium 또는 설정된 클라우드
브라우저 backend를 사용하고, task_id별 세션을 분리해 같은 작업 흐름 안에서
페이지 상태를 이어서 다룬다.
"""
...
from app.tools.web_runtime.support.auxiliary_client import call_llm
...
try:
    from app.tools.web_runtime.url_safety import is_safe_url as _is_safe_url
except Exception:
    _is_safe_url = lambda url: False  # noqa: E731 — fail-closed: block all if safety module unavailable
from app.tools.web_runtime.browser_providers.base import CloudBrowserProvider
from app.tools.web_runtime.browser_providers.browserbase import BrowserbaseProvider
from app.tools.web_runtime.browser_providers.browser_use import BrowserUseProvider
from app.tools.web_runtime.browser_providers.firecrawl import FirecrawlProvider
from app.tools.web_runtime.tool_backend_helpers import normalize_browser_cloud_provider

# Camofox local anti-detection browser backend (optional).
# When CAMOFOX_URL is set, all browser operations route through the
# camofox REST API instead of the agent-browser CLI.
try:
    from app.tools.web_runtime.browser_camofox import is_camofox_mode as _is_camofox_mode
except ImportError:
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    heygent_home = get_tool_home()
    heygent_node_bin = str(heygent_home / "node" / "bin")
    return [heygent_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_command_timeout() -> int:
    ...
    try:
        from app.tools.web_runtime.support.config import read_raw_config
        cfg = read_raw_config()
...
from app.tools.web_runtime.support.constants import is_termux as _is_termux_environment
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-heygent_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed 도구 실행 프로세스 — this way every clean 도구 실행 종료 sweeps
    ...
    # Sweep orphans from other crashed 도구 실행 프로세스.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live 도구 실행 프로세스.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current 도구 실행 PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live 도구 실행 프로세스
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning 도구 실행 프로세스 is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced 도구 실행 PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent 도구 실행 인스턴스 won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-heygent_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live 도구 실행 프로세스.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, tool-managed
    ...
    # Build an extended search PATH including tool-managed Node, macOS
...
def _run_browser_command(
    ...
    try:
        ...
        # Record this 도구 실행 PID as the session owner (cross-process safe
...
def _extract_relevant_content(
    ...
    from app.tools.web_runtime.support.redact import redact_sensitive_text
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    import urllib.parse
    from app.tools.web_runtime.support.redact import _PREFIX_RE
    ...
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_navigate
...
def browser_snapshot(
    ...
    """
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_snapshot
...
def browser_click(ref: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_click
...
def browser_type(ref: str, text: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_type
...
def browser_scroll(direction: str, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_scroll
...
def browser_back(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_back
...
def browser_press(key: str, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_press
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_console
...
def _camofox_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    from app.tools.web_runtime.browser_camofox import _ensure_tab, _post
    try:
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = heygent_home / "browser_recordings"
...
def browser_get_images(task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_get_images
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    """
    if _is_camofox_mode():
        from app.tools.web_runtime.browser_camofox import camofox_vision
    ...
    from app.tools.web_runtime.support.constants import get_tool_dir
    screenshots_dir = get_tool_dir("cache/screenshots", "browser_screenshots")
    ...
    try:
        ...
        try:
            ...
            from app.tools.web_runtime.vision_tools import (
...
def cleanup_browser(task_id: Optional[str] = None) -> None:
    ...
    if _is_camofox_mode():
        try:
            from app.tools.web_runtime.browser_camofox import camofox_close, camofox_soft_cleanup
...
if __name__ == "__main__":
    ...
    print("\n💡 Usage:")
    print("  from app.tools.web_runtime.browser_tool import browser_navigate, browser_snapshot")
