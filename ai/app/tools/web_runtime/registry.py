"""웹 검색/브라우저 실행 모듈이 내부 도구 목록을 모아 두는 registry다.

각 실행 모듈은 import될 때 schema, handler, toolset, 사용 가능 여부를
등록한다. 이 파일은 그 목록을 한곳에서 조회하고, 필요한 경우 같은
패키지 안의 실행 모듈들을 동적으로 불러오는 역할만 맡는다.
"""
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    module_names = [
        f"app.tools.web_runtime.{path.stem}"
...
#
# Usage:
#   from app.tools.web_runtime.registry import registry, tool_error, tool_result
#
