"""웹/브라우저 도구가 접근해도 되는 URL인지 확인하는 정책 모듈이다.

설정 파일의 차단 목록을 읽고, 외부 요청을 보내기 전에 같은 규칙으로
검색 추출 도구와 브라우저 도구의 접근 가능 여부를 판단한다.
"""
...
# URL check cache prevents repeated YAML parses during multi-page fetch flows.
...
def _get_default_config_path() -> Path:
    return get_tool_home() / "config.yaml"
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    for shared_file in raw_shared_files:
        ...
        if not path.is_absolute():
            path = (get_tool_home() / path).resolve()
