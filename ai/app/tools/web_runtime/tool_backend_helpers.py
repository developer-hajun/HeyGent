...
def managed_nous_tools_enabled() -> bool:
    ...
    """
    try:
        from app.tools.web_runtime.support.auth import get_nous_auth_status

        status = get_nous_auth_status()
        if not status.get("logged_in"):
            return False

        from app.tools.web_runtime.support.models import check_nous_free_tier
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and the optional tool config file so runtime
    checks and setup-time checks agree. A whitespace-only value is treated as
    unset everywhere.
    """
    ...
    if value is None:
        ...
        try:
            from app.tools.web_runtime.support.config import get_env_value
