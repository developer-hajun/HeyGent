...
class _FakeMemoryRecallPlannerClient:
    """테스트에서 실제 LLM 호출 없이 memory recall planning을 스킵하는 fake."""

    async def plan_memory_recall_json(self, **_kwargs):
        raise RuntimeError("memory recall planner disabled in test")
...
def _patch_app_runtime(app_main, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # ProviderMemoryRecallPlannerClient를 모킹하지 않으면 _patch_respond가 심은 mock 응답을
    # memory recall planner가 소비해 실제 태스크 루프에 전달될 응답 순서가 어긋난다.
    monkeypatch.setattr(app_main, "ProviderMemoryRecallPlannerClient", lambda **_kwargs: _FakeMemoryRecallPlannerClient())
