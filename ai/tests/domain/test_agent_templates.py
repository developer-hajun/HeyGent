...
def test_builtin_subagent_templates_default_to_worker_model():
    ...
    # 일부 특수 에이전트(gmail_agent 등)는 gpt-5.4를 쓸 수 있다.
    # 그 외 범용 서브에이전트는 비용 효율을 위해 gpt-5.2를 사용한다.
    allowed_worker_models = {"gpt-5.2", "gpt-5.4"}
    for template in BUILTIN_AGENT_TEMPLATES:
        assert template.model in allowed_worker_models
...
def test_builtin_subagent_templates_do_not_default_to_notion_or_mattermost():
    # gmail_agent는 메일 전달 기능을 위해 mattermost-send를 의도적으로 포함한다.
    restricted_defaults = {"notion", "mattermost-send"}
    specialized_exceptions = {"gmail_agent"}

    for template in BUILTIN_AGENT_TEMPLATES:
        if template.template_key in specialized_exceptions:
            continue
