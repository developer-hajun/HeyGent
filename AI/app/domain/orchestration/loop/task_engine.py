...
from app.domain.orchestration.delegation import DelegateRuntime
...
class TaskEngine:
    ...
    async def _execute(self, *, task: TaskRun, step: StepRun, executor, resume_payload: dict | None) -> TaskRun:
        ...
        outcome = normalize_executor_outcome(self.step_executor.execute(handler=executor, task=task, step=step, resume_payload=resume_payload))
