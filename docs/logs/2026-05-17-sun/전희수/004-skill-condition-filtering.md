...
- Hermes 레퍼런스처럼 skill 설명 주입 단계에서 실제 사용 가능한 runtime tool 기준의 optional condition 필터를 적용한다.
...
- capability resolver가 Hermes식 `requires_toolsets` metadata도 인식하도록 보완했다.
...
- 모든 skill에 `requires_tools`를 강제하지 않고, 레퍼런스처럼 조건 metadata가 있는 skill만 필터링한다.
