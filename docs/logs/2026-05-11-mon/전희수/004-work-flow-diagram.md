...
- Flow 맵 렌더링을 전용 그래프 렌더러 기반으로 전환해 노드와 엣지를 같은 좌표계에서 표시하도록 정리했다.
- Flow 맵은 화면 폭에 맞춰 축소하지 않고 고정 캔버스를 유지하며, 좁은 화면에서는 맵 영역 안에서 가로로 확인하도록 변경했다.
- Flow 맵의 작업 카드 폭과 카드 간격을 줄이고, 맵 카드 안의 실행 버튼은 제거해 작업 상세 진입 후 실행하도록 정리했다.
- 작업 간 실행 순서는 두 작업을 선택한 뒤 방향을 고르는 모달로 확정하도록 변경했다.
- 실행 순서선은 작업 노드 하단 전용 핸들과 별도 레인을 사용해 CEO/하위 작업 계층선과 겹치지 않게 분리했다.
- 선행 작업이 남아 있는 작업은 실행 화면으로 넘기기 전에 안내문을 띄우도록 정리했다.
- 하위 작업 생성과 세션 에이전트 작업 도구의 기본 동작은 부모 작업 상태를 즉시 차단으로 바꾸지 않도록 조정했다.
...
- `npm run lint`
- `npm run build`
- `python -m pytest ai/tests/api/test_openapi_auth.py ai/tests/storage/test_postgres_durable_contracts.py ai/tests/tools/test_runtime_tools.py -q`
...
- `http://localhost:5173` Flow 화면에서 작업 카드, 추가 슬롯, 실행 순서 도구, 작업 상세 진입 경로를 확인했다.
- `http://localhost:5173` Flow 화면에서 하위 작업 5개와 여러 실행 순서선을 표시해 카드 폭, 추가 슬롯 클릭, 하단 순서선 레인, 좁은 화면 가로 스크롤을 확인했다.
...
- 실행 순서 관계는 생성 시 상태를 즉시 `blocked`로 바꾸지 않고, 실행 시점에 사용자가 선행 작업을 완료하도록 안내하는 방향으로 둔다.
