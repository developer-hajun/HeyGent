...
Notion 스킬이 여러 reference 문서를 갖는 구조로 확장되면서, 사용자에게 로컬 경로/타입 같은 개발자 메타 정보가 아니라 서비스형 스킬 문서 탐색 UI를 제공한다.

# 변경 요약

- Notion 스킬 본문을 진입점 중심으로 정리하고, 외부 레퍼런스 기반의 상세 문서를 `references/` 아래에 추가했다.
...
- 스킬 런타임은 여전히 `SKILL.md`를 진입점으로 사용하고, 세부 내용은 reference 문서를 필요할 때 읽는 구조로 유지한다.

# 다음 단계

- 다른 스킬에도 reference 문서가 늘어나면 같은 문서 탐색 UI를 재사용한다.
